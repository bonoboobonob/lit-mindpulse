    ...
    <div className="w-full max-w-3xl mx-auto px-2 sm:px-4 py-2 flex flex-col items-center">
      ...
      <div className="w-full flex items-center justify-between mb-4 gap-2">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          {phase === 'recalling' && (
            <div className="flex items-center gap-1 px-2.5 sm:px-3 py-1 bg-[#FAF6EE] dark:bg-[#24201C] border border-[#D6CEBE] dark:border-[#38322B] rounded-xl text-[#B44A22] dark:text-[#E07048] font-bold text-xs shrink-0 shadow-xs">
      ...
      <div className="text-center mb-6">
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#8C5E3C]/10 dark:bg-[#D4AF37]/15 border border-[#8C5E3C]/30 dark:border-[#D4AF37]/40 text-[#8C5E3C] dark:text-[#D4AF37] text-xs font-bold uppercase tracking-wider mb-2">
              ...
              {BOOK_GENRES.map((g) => (
                <button
                  ...
                  className={`p-3 rounded-xl border text-left transition flex flex-col justify-between cursor-pointer ${
                    selectedGenre === g.id
                      ? 'bg-[#8C5E3C]/15 dark:bg-[#D4AF37]/20 border-[#8C5E3C] dark:border-[#D4AF37] text-[#8C5E3C] dark:text-[#D4AF37] font-bold shadow-xs'
                ...
                <button
                  ...
                  className={`p-3.5 rounded-2xl border text-left transition cursor-pointer ${
                    selectedDifficulty === d.id
                      ? 'bg-[#8C5E3C]/10 dark:bg-[#D4AF37]/20 border-[#8C5E3C] dark:border-[#D4AF37] text-[#1C1917] dark:text-[#F5EFE4] shadow-xs'
        ...
        <div className="w-full max-w-2xl flex flex-col items-center animate-in fade-in duration-200">
          ...
          <div className="w-full flex items-center justify-between gap-2 mb-3 p-2 rounded-2xl bg-white dark:bg-[#1C1917] border border-[#D6CEBE] dark:border-[#38322B] shadow-xs text-xs">
            <div className="flex items-center gap-1.5 overflow-x-auto py-0.5">
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-[#FAF6EE] dark:bg-[#282420] border border-[#D6CEBE] dark:border-[#38322B] text-[#57534E] dark:text-[#A8A196] hover:text-[#1C1917] dark:hover:text-[#F5EFE4] hover:border-[#8C5E3C] font-semibold transition cursor-pointer shrink-0"
              >
                <span>Tür: <strong className="text-[#8C5E3C] dark:text-[#D4AF37]">{BOOK_GENRES.find(g => g.id === selectedGenre)?.name || 'Tüm Türler'}</strong></span>
              </button>
                  ...
                  <button
                    ...
                    className={`px-2 py-0.5 rounded-lg text-[11px] font-bold transition cursor-pointer ${
                      selectedDifficulty === d
                        ? 'bg-[#8C5E3C] dark:bg-[#D4AF37] text-white dark:text-[#1C1917] shadow-xs'
            ...
            <button
              onClick={() => startNewRound()}
              className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-[#FAF6EE] dark:bg-[#282420] border border-[#D6CEBE] dark:border-[#38322B] text-[#8C5E3C] dark:text-[#D4AF37] font-bold text-xs hover:bg-[#F2ECE1] transition cursor-pointer shrink-0"
          ...
          <div className="w-full relative p-6 sm:p-8 rounded-3xl bg-white dark:bg-[#1C1917] border border-[#D6CEBE] dark:border-[#38322B] shadow-md mb-8">
            <div className="flex items-center justify-between text-xs text-[#57534E] dark:text-[#A8A196] mb-4 pb-3 border-b border-[#D6CEBE] dark:border-[#38322B]">
              ...
              <span className="px-2.5 py-0.5 rounded-full bg-[#FAF6EE] dark:bg-[#282420] text-[#8C5E3C] dark:text-[#D4AF37] border border-[#D6CEBE] dark:border-[#38322B] text-[11px] font-bold">
                {words.length} Kelime
              </span>
            </div>
        ...
        <div className="w-full max-w-2xl flex flex-col items-center animate-in fade-in duration-200">
          <div className="w-full flex items-center justify-between p-3.5 rounded-2xl bg-white dark:bg-[#1C1917] border border-[#D6CEBE] dark:border-[#38322B] mb-4 text-xs shadow-xs">
            <button
              ...
              title="Kitabı İncele (Egzersiz duraklatılır)"
            >
              <BookOpen className="w-4 h-4 text-[#8C5E3C] dark:text-[#D4AF37] shrink-0" />
              <strong className="text-[#8C5E3C] dark:text-[#D4AF37] font-serif truncate underline decoration-dotted underline-offset-2">{currentQuote.book}</strong>
          ...
          {isPeeking && (
            <div className="w-full p-4 rounded-2xl bg-[#FAF6EE] dark:bg-[#282420] border-2 border-[#8C5E3C] dark:border-[#D4AF37] text-[#8C5E3C] dark:text-[#D4AF37] text-center font-serif text-base italic mb-4 animate-in fade-in font-quote">
              "{currentQuote.quote}"
            </div>
          )}
                ...
                return (
                  <span
                    ...
                    className={`inline-flex items-baseline px-2 py-1 rounded-xl transition cursor-pointer ${
                      isCurrent
                        ? 'bg-[#C85A32]/15 dark:bg-[#E07048]/20 border-2 border-[#C85A32] dark:border-[#E07048] scale-105 shadow-xs'
                        : isPast
                        ? isCorrect
                  ...
                  >
                    {w.prefix}
                    <strong className="text-lg sm:text-2xl text-[#B44A22] dark:text-[#E07048] font-serif font-bold">
                      {w.firstLetter}
                    </strong>
          ...
          <div className="w-full max-w-md flex flex-col items-center mb-6">
            <div className="w-full flex items-center gap-2">
              <input
                ...
                className="flex-1 p-4 rounded-2xl bg-white dark:bg-[#1C1917] border-2 border-[#8C5E3C] dark:border-[#D4AF37] focus:border-[#C85A32] dark:focus:border-[#E07048] text-[#1C1917] dark:text-[#F5EFE4] placeholder-[#78716C] dark:placeholder-[#A8A196] text-base sm:text-lg font-serif text-center focus:outline-none shadow-sm transition-all"
                autoFocus
              />
              <button
                onClick={() => commitCurrentWord(currentWordInput.trim())}
                className="px-5 py-4 rounded-2xl btn-terracotta font-bold text-sm shadow-md transition cursor-pointer shrink-0"
              >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#588157] hover:bg-[#476C46] text-white text-xs font-bold transition cursor-pointer shadow-xs"
            >
            ...
            <div className="flex flex-wrap gap-2 mb-6 p-4 rounded-2xl bg-[#FAF6EE] dark:bg-[#24201C] border border-[#D6CEBE] dark:border-[#38322B]">
              {words.map((w, idx) => {
                ...
                return (
                  ...
                  >
                    <span className="font-bold text-[#B44A22] dark:text-[#E07048] mr-1">[{w.firstLetter}]</span>
                    <span>{w.core}</span>
                    {!isCorrect && user && (
